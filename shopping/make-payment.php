<?php
    ...
    if(empty($_SESSION['is_customer_login'])){
        ...
        $orderPrice = 0;
        $resOrderData = mysqli_query($conn, "SELECT product_master.Price, product_master.Discount, ecom_sales_temp.Quantity FROM ecom_sales_temp JOIN product_master ON product_master.PM_Id = ecom_sales_temp.ProductId WHERE ecom_sales_temp.OrderId = '$orderId'");
        if(mysqli_num_rows($resOrderData)>0){

            while($rowOrderData = mysqli_fetch_assoc($resOrderData)){

                $productPrice = ($rowOrderData['Price'] - ($rowOrderData['Price'] * ($rowOrderData['Discount'] / 100)) * $rowOrderData['Quantity']);
            ...
            if($orderPrice > 0){
                ...
                $SUCCESS_DOMAIN = "http://pawsnap.in/shopping/payment-success.php?success=".$orderId;
                $FAIL_DOMAIN = "http://pawsnap.in/shopping/payment-failed.php?failed=".$orderId;
                ...
                if(mysqli_query($conn, "INSERT INTO payment_master (OrderId, TransactionId, PaidCurrency, PaymentStatus, DatePaid, TotalAmount, 
                    ...
                    echo "<script>alert('Oops, Unable to process..');location.href='payment-error.php?pref=$message';</script>";
                }
            } else {
