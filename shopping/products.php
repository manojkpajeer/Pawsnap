<?php
    ...
    } else if($pref == 'Personalised'){
        
        $sql = "SELECT COUNT(product_master.PM_Id) AS total, category_master.CategoryName, category_master.CT_Id FROM product_master JOIN 
                category_master ON category_master.CT_Id = product_master.CategoryId WHERE category_master.Status = 1 
                AND product_master.Status = 1 AND category_master.ParentId = 3 GROUP BY category_master.CategoryName, category_master.CT_Id";
                
    }

    $sql2 = "SELECT category_master.ParentId, product_master.* FROM product_master JOIN category_master ON category_master.CT_Id = product_master.CategoryId WHERE product_master.CategoryId = '$source' AND product_master.OnlineDiscount >= '$discount' ";
                                                ...
                                                <?php
                                                    while($rowPrCategory = mysqli_fetch_assoc($resPrCategory)){
                                                        ...
                                                        <li class="list-group-item d-flex justify-content-between align-items-center">
                                                            <a href="products.php?pref=<?php echo $pref;?>&source=<?php echo $rowPrCategory['CT_Id']; ?>&sort=<?php echo $sort;?>&discount=<?php echo $discount;?>&page=<?php echo $page;?>">
                                                                <?php if($rowPrCategory['CT_Id'] == $source){ echo "<strong class='text-danger'>".$rowPrCategory['CategoryName']."</strong>";}else {echo $rowPrCategory['CategoryName'];}?>
                                                            </a>
