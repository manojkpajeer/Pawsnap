<?php 
    ...
    $sql= "select category_master.CategoryName as cname, product_master.PM_Id, product_master.ProductName, product_master.ProductCode from product_master join category_master on category_master.CT_Id = product_master.CategoryId order by product_master.PM_Id desc;";
    ?>
                        ...
                        if(mysqli_num_rows($resData)>0)
                        {
                            ...
                            while($row = mysqli_fetch_assoc($resData))
                            {
                                ...
                                <tr>
                                    ...
                                    <td><?php $res1 = mysqli_query($conn, "select sum(quantity) as total_quantity from purchase_temp where product_id = '$row[PM_Id]' AND status = 1");
                                                $row1 = mysqli_fetch_assoc($res1);

                                                $res2 = mysqli_query($conn, "select sum(quantity) as sale_quantity from sales_temp where product_id = '$row[PM_Id]' AND status = 1");
