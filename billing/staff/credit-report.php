<?php 
    ...
    $sql= "SELECT billing_customer.FullName, billing_customer.PhoneNo, sales.billno, sales.bill_date, sales.paid, sales.remark, sales.discount FROM sales JOIN billing_customer ON billing_customer.BC_Id = sales.customer_id WHERE sales.sales_status = 1 ORDER BY sales.id DESC";
    if (isset($_POST['filter'])) {
        if (!empty($_POST['fromdate']) && !empty($_POST['todate'])) {
            $sql= "SELECT billing_customer.FullName, billing_customer.PhoneNo, sales.billno, sales.bill_date, sales.paid, sales.remark, sales.discount FROM sales JOIN billing_customer ON billing_customer.BC_Id = sales.customer_id WHERE sales.sales_status = 1 AND (bill_date BETWEEN '$_POST[fromdate]' AND '$_POST[todate]') ORDER BY sales.id DESC";
        }
    } 
    ?>
    ...
    <div class="pcoded-content">
        <div class="card">
            <div class="card-header pb-0 row">
                <div class="col-5"><h4>Creadit Report</h4></div>
                    ...
                    <table id="dom-jqry" class="table table-hover table-bordered nowrap ">
                        ...
                        if(mysqli_num_rows($resData)>0)
                        {
                            ...
                            while($row = mysqli_fetch_assoc($resData))
                            {
                                ...
                                $pending_amount = $final_total - $row['paid'];
                                ...
                                if ($pending_amount > 1) {
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right"><?php echo number_format($pending_amount, 2); ?></td>
